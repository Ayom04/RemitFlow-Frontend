...
describe('Chart component', () => {
  ...
  describe('multi-series with legend', () => {
    const seriesData = [
      {
        name: 'Sent',
        color: '#6366f1',
        data: [{ value: 200 }, { value: 120 }],
      },
      {
        name: 'Received',
        color: '#10b981',
        data: [{ value: 180 }, { value: 100 }],
      },
    ];
    ...
    it('dims legend item when series is hidden', async () => {
      ...
      expect(
        sentButton.closest('.chart-legend-item--dimmed'),
      ).toBeInTheDocument();
    ...
    it('does not render legend when using data prop', () => {
      const { container } = render(
        <Chart data={[{ value: 10 }]} title="Single" />,
      );
    ...
    it('uses default colors when series omit color', () => {
      ...
      const { container } = render(
        <Chart series={seriesNoColor} title="Colors" />,
      );
      const bars = container.querySelectorAll('.chart-bar');
